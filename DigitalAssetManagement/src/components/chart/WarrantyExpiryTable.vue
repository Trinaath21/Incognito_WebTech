<template>
    <div class="warranty-expiry-table">
      ...
      <div class="mb-4 flex flex-wrap gap-4">
        ...
        <el-select v-model="filterPeriod" placeholder="Expiry Period">
          <el-option v-for="period in expiryPeriods" :key="period.value" :label="period.label" :value="period.value" />
        </el-select>
      </div>
      ...
      >
        ...
        <el-table-column prop="daysRemaining" label="Days Remaining" width="150" sortable>
          <template #default="{ row }">
            <el-tag 
              :type="getStatusType(row.daysRemaining)" 
              size="small"
              effect="light"
            >
  ...
  const filteredAssets = computed(() => {
    ...
    // Filter by warranty expiry period
  ...
  const openEditForm = () => {
    selectedAsset.value = null // Reset selected asset for general update
